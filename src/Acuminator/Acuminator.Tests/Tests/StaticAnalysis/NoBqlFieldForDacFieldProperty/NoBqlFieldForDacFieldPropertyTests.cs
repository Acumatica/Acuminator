...
namespace Acuminator.Tests.Tests.StaticAnalysis.NoBqlFieldForDacFieldProperty
{
	...
	{
		[Theory]
		[EmbeddedFileData(@"MissingBqlField\DacWithoutBqlFields.cs")]
		...
		[Theory]
		[EmbeddedFileData(@"MissingBqlField\DacWithoutBqlFields_Expected.cs")]
		public async Task RegularDac_WithBqlFields_AfterFix_ShouldNotShowDiagnostic(string actual) =>
			await VerifyCSharpDiagnosticAsync(actual);

		[Theory]
		[EmbeddedFileData(@"MissingBqlField\DacWithBqlFieldMissingInBaseDac_Expected.cs")]
		public async Task DerivedDac_BqlFieldInBaseDac_AfterFix_ShouldNotShowDiagnostic(string actual) =>
			await VerifyCSharpDiagnosticAsync(actual);

		[Theory]
		[EmbeddedFileData(@"MissingBqlField\DacExtensionWithBqlFieldMissingInBaseDac_Expected.cs")]

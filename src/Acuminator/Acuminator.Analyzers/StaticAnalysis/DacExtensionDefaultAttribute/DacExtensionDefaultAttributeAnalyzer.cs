...
namespace Acuminator.Analyzers.StaticAnalysis.DacExtensionDefaultAttribute
{
	...
	public class DacExtensionDefaultAttributeAnalyzer : PXDiagnosticAnalyzer
	{
        ...
        //-----------------------------------------------------------------------------------------------------------------------------------------------
        /*private static void CheckDacExtensionPropertyOld(IPropertySymbol property, SymbolAnalysisContext symbolContext, PXContext pxContext)
		{
            symbolContext.CancellationToken.ThrowIfCancellationRequested();
			...
			bool isBoundField = attributeInformation.ContainsBoundAttributes(propertyAttributes);

			if (isBoundField)
			{
				AnalyzeAttributesWithinBoundField(property, propertyAttributes, pxContext, symbolContext, attributeInformation);
			}
			else if (!isBoundField)
			{
				AnalyzeAttributesWithinUnBoundField(property, propertyAttributes, pxContext, symbolContext, attributeInformation);
			}

		}*/

		/*private static void AnalyzeAttributesWithinBoundField(
            IPropertySymbol property, ImmutableArray<AttributeData> attributes,
            PXContext pxContext,
            SymbolAnalysisContext symbolContext,
            AttributeInformation attributeInformation)
		{
            if (!property.ContainingType.IsDacExtension(pxContext))
            {
                return;
            }

            var pxDefaultAttributes = attributes
                .Where(a => attributeInformation.IsAttributeDerivedFromClass(a.AttributeClass, pxContext.AttributeTypes.PXDefaultAttribute));

			foreach (var attribute in pxDefaultAttributes)
            {
                var isPersistingCheckNothing = attribute.NamedArguments
                    .Where(a => IsAttributeContainsPersistingCheckNothing(a))
                    .Any();
                if (isPersistingCheckNothing)
                {
                    return;
                }

                Location attributeLocation = GetAttributeLocation(attribute, symbolContext.CancellationToken);
                if (attributeLocation == null)
                {
                    continue;
                }
		...
		/*private static void AnalyzeAttributesWithinUnBoundField(IPropertySymbol property, ImmutableArray<AttributeData> attributes,

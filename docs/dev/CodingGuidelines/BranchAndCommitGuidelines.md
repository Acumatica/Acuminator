...
When creating a new branch for a feature or a bug fix, follow the guidelines below to name your branch.
1. The branch name should be descriptive and concise. Add extra details to the name of the branch if needed.
2. The branch name should start with the type of the branch that defines its purpose:
   - **feature**
   - **bugfix**
...
1. Try to keep the related changes together in a single commit. This is the most subjective rule here, so use your best judgment to decide what changes are related. 
2. Keep commits granular. If you have a large change that can be logically split into smaller parts, break it into multiple commits. Do not commit it as a single commit.
   This will make the investigation of the history with Git blame easier.
3. If you have significant formatting-only changes (for example, fixing indentation in the file), do not mix them with functional changes. It is better to have a separate commit for the formatting changes
   and mention that the commit contains formatting changes in the commit message.
4. Create the commit message according to the guidelines described in the next section.

## Commit Message General Guidelines

 - Commit messages should be clear and concise so that other developers can understand the context of the changes without looking at the code.
   Follow these guidelines when writing commit messages:
    - Describe what was changed and why, rather than how it was implemented.
    - Describe the general scope of the changes instead of specific code modifications.
    - Avoid repeating the same description across a series of related commits.
    Examples:
    ```
    Added additional check for PXStringList
    Fixed typo in the guildelines
    ```

 - A commit message should contain at least 16 characters.
 
 - If the commit is related to an item in the Acumatica internal bug tracking system, the commit message should start with the bug or feature identifier followed by a semicolon and the message. The recommended format is:
   ```

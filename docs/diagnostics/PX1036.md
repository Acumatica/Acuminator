...
The diagnostic checks the correct naming of a primary and foreign key field declarations in DACs. For a primary key, the name should be `PK`. For a foreign key, the name should be `FK`.
...
The code fix suggests that the developer changes the name of the primary key class to `PK`, and the name of the foreign key to `FK`.

...
You should use message strings from the classes with the `PX.Common.PXLocalizableAttribute` attribute instead of hardcoded strings in the following cases:
 ...
 The diagnostic shows an error for the following strings:
 - Hardcoded strings including interpolated strings 
 - Strings passed to localization methods or exception constructors that are not constant fields

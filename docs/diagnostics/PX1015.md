...
For BQL statements that contain `Required` and `Optional` parameters, a `Select` method must have the number of arguments that is equal to or greater than the number of `Required` parameters in the BQL statement. 

To fix the issue, check that all necessary arguments are passed to the method.

...
To fix the issue, you should remove all primary, foreign, or unique key declarations except one.
...
An example of a DAC with multiple primary key declarations which use the same set of fields is shown in the following code.
```C#
...
An example of a unique key declaration 

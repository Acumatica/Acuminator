...
An exception thrown in a `RowPersisted` event handler prevents saving of the data to the database but does not rallback changes in `PXCache` instances, which leads to data inconsistency.

To fix the issue, you should remove the code that throws the exception and rework the related business logic. If you need to throw the exception to prevent saving of the record, you should use the `RowPersisting` event handler instead.

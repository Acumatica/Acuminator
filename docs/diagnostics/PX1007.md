...
Any public class, interface, struct, enumeration, delegate, or their member that is defined in a `PX` namespace (such as the `PX.Objects.AR` namespace) should have a description in the `summary` XML tag. These descriptions can be used for building the API and DAC references for the Acumatica ERP libraries.

...
The diagnostic is not displayed for the following items: 
 - Fields, properties, and methods of public entities (except DAC property fields).
 - Child classes if their parent class has the `exclude` XML tag.
 - DACs with the `PXHidden`, `PXInternalUseOnly`, or `Obsolete` attribute.
 - Class fields of DACs. In DACs, XML annotations should be specified only for property fields.
 - System fields of DACs, such as `CreatedDateTime` or `LastModifiedDateTime`.
 - Public items from the assemblies whose names contain `Test` or `Benchmark`.
